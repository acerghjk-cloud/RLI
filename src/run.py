...
def main(args):
    ...
    pjf_path = "./lora"
    # pipeline.load_lora_weights(pjf_path, weight_name="pytorch_lora_weights.safetensors")
    ...
    if verbose:
        ...
        image_inv, x_t = run_and_display(pipeline,neg_prompts,prompts, controller, run_baseline=False, latent=x_t, uncond_embeddings=uncond_embeddings, uncond_embeddings_p=uncond_embeddings_p,verbose=False)
        ptp_utils.view_images([image_gt, image_enc, image_inv[0]])
        ptp_utils.save_individual_images([image_gt, image_enc, image_inv[0]])
        show_cross_attention(pipeline,prompts,controller, 32, ["up","down","mid"])
    else:
    ...
    torch.cuda.empty_cache()
    gc.collect()
    controller = make_controller(pipeline,prompts, True, cross_replace_steps, self_replace_steps, blend_word, eq_params, blend_word)
    images, _ = run_and_display(pipeline,neg_prompts, prompts, controller, run_baseline=False, latent=x_t, uncond_embeddings=uncond_embeddings,uncond_embeddings_p=uncond_embeddings_p, steps=50)
...
if __name__ == '__main__':
    ...
    p.add_argument("--image_path", type=str, default="./img/d[0048]TopBF1.png", help="Image Path") 
